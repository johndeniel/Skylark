...
import {WritingListProps, WritingItem} from "@/types/index"


const dateWithDayAndMonthFormatter = new Intl.DateTimeFormat('tr-TR', {
  day: '2-digit',
  month: '2-digit'
});

const dateWithMonthAndYearFormatter = new Intl.DateTimeFormat('en-US', {
  month: '2-digit',
  year: 'numeric'
});

const WritingList: React.FC<WritingListProps> = ({ items, viewCounts }) => {

  const itemsEntriesByYear: [number, WritingItem[]][] = items.reduce((acc, item) => {
    
      const year = new Date(item.date).getFullYear();
      const yearArr = acc.find((entry) => entry[0] === year);
      if (!yearArr) {
        acc.push([year, [item]]);
      } else {
        yearArr[1].push(item);
          ...
          return (
            <ul className="list-none" key={year}>
              {itemsArr.map((item, itemIndex) => {
                ...
                const views =
                  viewCounts?.find((item) => item.slug === slug)?.view_count || 0;
                ...
                return (
                  <li key={slug} className="grid grid-cols-6 p-0 group-hover:text-gray-300">
                    ...
                    <Link href={`/writing/${slug}`} passHref className="col-span-6 hover:text-black md:col-span-5">
